...
export default {
  ...
  methods: {
    getList (year) {
      this.$store.dispatch('getSumReportInDepartment', year).then((res) => {
        this.loading = false
    ...
    getItemInfo (item) {
      this.$router.push({ name: 'depOne', query: { year: item.setYear, departmentCode: item.departmentCode, name: item.departmentName } })
    }
  }
}
</script>
